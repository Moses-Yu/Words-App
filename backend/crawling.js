...
const searchAndAddWord = async (keyword) => {
  ...
  if (!existing) {
    ...
    if (id == undefined) {
      ...
      //   console.log($1(".cont_detail.cont_kuek").html());
      //   $1(".fold_ex").each((idx, node) => {
      //     word.type = word.type + $(node).find(".tit_ex").text() + " ";
      //     word.meaning_deep =
      //       word.meaning_deep + $(node).find(".desc_item").text() + " ";
      //   });
...
const run = async () => {
  ...
  for (let index = 0; index < words.length; index++) {
    ...
    await searchAndAddWord(words[index]);
  }
  console.log(words);
};
